-- Invariant 1 (Identity 2): closing_base[t] = opening_base[t] + net
-- movement, and opening_base[t] = closing_base[t-1]. Recomputed
-- independently via lag() so this catches a broken fct_base, not just
-- restates it. Passes (returns zero rows) when the identity holds
-- everywhere.
with base as (
    select
        node,
